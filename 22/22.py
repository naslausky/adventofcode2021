...
def intervaloEstaContidoEmOutro(menor, maior): # Função que verifica se um intervalo (a,b) está incluso em outro.
	if menor[0] >= maior[0] and menor[1] <= maior[1] + 1: # Somar 1 foi necessário pois "maior" é utilizado vindo direto das instruções de entrada.
		return True
	return False
...
for indiceX in range(len(intervalosOrdenados[0]) - 1): # Verifica todas as combinações de intervalos possíveis.
	...
	if not iX: # Se não existe nenhuma instrução que afete esse intervalo, não tem porque seguir com ela. Ex: Instruções (1, 2) e (3, 4) não afetam o intervalo (2,3).
		continue
	for indiceY in range(len(intervalosOrdenados[1]) - 1):
		...
		if not iY:
			continue # Como acima, não tem porque iterar sobre todos os intervalos Z se nenhuma instrução afeta essa combinação de intervalos X e Y.
		for indiceZ in range(len(intervalosOrdenados[2]) - 1):
			...
			if intervaloAceso:
				...
				if intervaloEstaContidoNaZonaDeInicializacao((rangeX,rangeY,rangeZ)): # Soma separada para a primeira parte.
